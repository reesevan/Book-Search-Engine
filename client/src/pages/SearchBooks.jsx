...
const SearchBooks = () => {
    ...
    return (<>
      ...
      <Container>
        ...
        {error && (<p className="text-danger">Error occurred while searching. Please try again.</p>)}

        <Row>
          {searchedBooks.map((book) => (<Col md="4" key={book.bookId}>
              <Card border="dark" className="mb-3">
                {book.image && (<Card.Img src={book.image} alt={`Cover for ${book.title}`} variant="top" style={{ maxHeight: '250px', objectFit: 'contain' }}/>)}
                <Card.Body>
                  <Card.Title>{book.title}</Card.Title>
                  <p className="small">Authors: {book.authors.join(', ')}</p>
                  <Card.Text>{book.description}</Card.Text>
                  {book.link && (<Card.Link href={book.link} target="_blank" rel="noopener noreferrer">
                      More Info
                    </Card.Link>)}
                </Card.Body>
              </Card>
